...
func (p *Processor) persistMiners(ctx context.Context, miners []minerActorInfo) error {
	...
	grp.Go(func() error {
		...
		r := splitWork(4, miners)
		dividedMiners, ok := r.([][]minerActorInfo)
		if !ok {
			return xerrors.Errorf("splitWork returned non-castable result: %+v", r)
		}
...
func splitWork(splitNum int, work interface{}) interface{} {
	workAsSlice, ok := work.([]interface{})
	if !ok {
		log.Error("splitWork on non-slice value")
		return work
	}
	var (
		splitWork [][]interface{}
		chunkSize = (len(workAsSlice) + splitNum - 1) / splitNum
	)
	if chunkSize == 0 {
		chunkSize = 1
	}

	for i := 0; i < len(workAsSlice); i += chunkSize {
		end := i + chunkSize

		if end > len(workAsSlice) {
			end = len(workAsSlice)
		}

		splitWork = append(splitWork, workAsSlice[i:end])
	}
	return splitWork
}
